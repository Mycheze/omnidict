...
export class CacheRepository {
  ...
  public async getCachedLemma(
    ...
    try {
      const statements = this.getStatements();
      const result = (await statements.getCachedLemma.get(
        word,
        targetLanguage,
      )) as { lemma: string } | undefined;
  ...
  public async getCacheStats(): Promise<{
    ...
    try {
      ...
      const hitRate =
        total.count > 0
          ? Math.max(0, (total.count - expired.count) / total.count)
          : 0;
  ...
  public async getCacheMetrics(): Promise<{
    ...
    try {
      ...
      const topLangsStmt = db.prepare(`
        ...
        LIMIT 5
      `);
      const topLangs = (await topLangsStmt.all()) as Array<{
        language: string;
        count: number;
      }>;
   ...
   */
  public async preloadFrequentLemmas(
    limit = 100,
    ...
    try {
      ...
      const stmt = db.prepare(`
        ...
        LIMIT ?
      `);
      const results = (await stmt.all(limit)) as Array<{
        word: string;
        lemma: string;
  ...
  public async bulkCacheLemmas(
    ...
    try {
      ...
      for (const item of lemmas) {
        try {
          await statements.setCachedLemma.run(
            item.word,
            item.lemma,
            item.targetLanguage,
          );
