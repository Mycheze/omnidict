...
describe("ContextSchema", () => {
  ...
  it("accepts valid context sentence", () => {
    expect(ContextSchema.parse("The cat sat on the mat.")).toBe(
...
describe("SearchRequestSchema", () => {
  ...
  it("rejects page out of range", () => {
    ...
    expect(() =>
      SearchRequestSchema.parse({ filters: {}, page: 101 }),
    ).toThrow();
  });

  it("rejects pageSize out of range", () => {
    expect(() =>
      SearchRequestSchema.parse({ filters: {}, pageSize: 0 }),
    ).toThrow();
    expect(() =>
      SearchRequestSchema.parse({ filters: {}, pageSize: 2001 }),
    ).toThrow();
  });
...
describe("validateOrigin", () => {
  ...
  it("returns false when host is missing", () => {
    expect(validateOrigin(makeRequest("http://localhost:3200", null))).toBe(
      false,
    );
  });
  ...
  it("allows same-origin http request", () => {
    expect(
      validateOrigin(makeRequest("http://localhost:3200", "localhost:3200")),
    ).toBe(true);
  });

  it("allows same-origin https request", () => {
    expect(
      validateOrigin(makeRequest("https://example.com", "example.com")),
    ).toBe(true);
  });

  it("allows localhost:3200 explicitly", () => {
    expect(
      validateOrigin(makeRequest("http://localhost:3200", "some-other-host")),
    ).toBe(true);
  });

  it("allows 127.0.0.1:3200 explicitly", () => {
    expect(
      validateOrigin(makeRequest("http://127.0.0.1:3200", "some-other-host")),
    ).toBe(true);
  });

  it("rejects cross-origin requests", () => {
    expect(
      validateOrigin(makeRequest("http://evil.com", "localhost:3200")),
    ).toBe(false);
  });
});
