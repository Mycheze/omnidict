...
export class AnkiConnect {
  ...
  private async makeDirectRequest(request: any): Promise<any> {
    try {
      ...
      if (!response.ok) {
        throw new AnkiConnectError(
          `AnkiConnect request failed: ${response.status}`,
        );
      }
  ...
  async testConnection(): Promise<AnkiConnectionStatus> {
    try {
      ...
      return {
        connected: false,
        error:
          error instanceof AnkiConnectError ? error.message : "Unknown error",
      };
    }
  }
  ...
  async getNoteTypes(): Promise<AnkiNoteType[]> {
    ...
    for (const name of noteTypeNames) {
      try {
        const fields = await this.makeRequest("modelFieldNames", {
          modelName: name,
        });
        ...
        console.warn(
          `Could not get fields for note type ${name}:`,
          errorMessage,
        );
      }
    }
  ...
  /**
   * Store a media file in Anki's collection (base64-encoded content).
   * Returns the filename Anki stored it under.
   */
  async storeMediaFile(filename: string, base64Data: string): Promise<string> {
    return await this.makeRequest("storeMediaFile", {
      filename,
      data: base64Data,
    });
  }
  ...
  async updateMostRecentNote(
    ...
    try {
      ...
      if (recentNotes.length === 0) {
        throw new AnkiConnectError(
          "No recent notes found in the specified deck",
        );
      }
