...
describe('aiStore', () => {
  ...
  it('sets model per provider', () => {
    useAIStore.getState().setSelectedModel('claude', 'claude-opus-4-5-20251101');
    expect(useAIStore.getState().selectedModels.claude).toBe('claude-opus-4-5-20251101');
  });

  it('does not affect other providers when setting model', () => {
    useAIStore.getState().setSelectedModel('claude', 'claude-opus-4-5-20251101');
    expect(useAIStore.getState().selectedModels.deepseek).toBe('deepseek-chat');
  });

  it('stores test results with timestamp', () => {
    const before = Date.now();
    useAIStore.getState().setTestResult('deepseek', { success: true, message: 'OK' });
